...
if len(customers) > 0:
    ...
    if edit_mode == 'Add' or (edit_mode == 'Edit' and _row_list):
        ...
        st.markdown(f"🛒 **SubTotal {inv_cur}**: {subtotal if subtotal != '-' else '-':.2f}")
        
        tax_amount = st.session_state.get('tax_amount', '-')
        st.markdown(f"🧾 **Tax Amount {inv_cur}**: {tax_amount if tax_amount != '-' else '-':.2f}")
        
        total = st.session_state.get('total', '-')
        st.markdown(f"💵 **Total {inv_cur}**: {total if total != '-' else '-':.2f}")
