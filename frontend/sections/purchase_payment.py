...
def update_pmt_item(entry: dict) -> dict:
    if (inv_num := entry.get('invoice_num')) is not None:
        ...
        balance = get_purchase_invoice_balance(inv_id, pmt_date, access_token=access_token)
...
def validate_payment(payment_: dict):
    ...
    payment_ = validate_purchase_payment(payment_, access_token=access_token)
    if isinstance(payment_, dict):
        if payment_.get('payment_items') is not None:
            st.session_state['validated'] = True
            
            # calculate and journal to session state
            jrn_ = create_journal_from_new_purchase_payment(payment_, access_token=access_token)
...
if len(suppliers) > 0:
    ...
    if edit_mode == 'Edit':
        ...
        if _row_list := selected['selection']['rows']:
            ...
            pmt_sel, jrn_sel = get_purchase_payment_journal(pmt_id_sel, access_token=access_token)
            ...
            pmt_items = [
                {
                    ...
                    'balance': get_purchase_invoice_balance(pi['invoice_id'], pmt_date, access_token=access_token)['balance'],
